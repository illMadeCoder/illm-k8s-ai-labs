...
│  │              experiments/components/core-app-of-apps.yaml          │   │
│  └───────────────────────────┬─────────────────────────────────────┘   │
│                              │                                          │
│       ┌──────────────────────┼──────────────────────┐                  │
│       │                      │                      │                  │
...
│  │                  experiments/scenarios/{name}/{cluster}/argocd/        │   │
│  ├─────────────────┬─────────────────┬─────────────────────────────┤   │
...
**File:** `experiments/components/core-app-of-apps.yaml`
...
**File:** `experiments/components/stacks/elk.yaml`
- Deploys ECK Operator + ELK Stack as a unit
- Used when full Elasticsearch logging is needed

**File:** `experiments/components/stacks/loki.yaml`
...
**Target Application** (`experiments/scenarios/http-baseline/target/argocd/app.yaml`):
```yaml
apiVersion: argoproj.io/v1alpha1
kind: Application
metadata:
...
spec:
  ...
  sources:
    ...
    - repoURL: https://github.com/illMadeCoder/illm-k8s-lab.git
      targetRevision: HEAD
      path: experiments/scenarios/http-baseline/target/argocd
      directory:
        recurse: false
